...
void loop(void) {
...
#ifdef RGB_BUILTIN
  leds[0] = CHSV(hue++, 255, 255);//rainbow
  FastLED.show();
#endif
#ifdef BUTTON_PIN
  ...
  if(api2Json.checkResult(uid, piccChallenge, piccResponse)) {
    ...
    //api2Json.storeKVP(piccChallenge, "USERNAME", SECRET_USERNAME);//only needs to be stored once.
    //Keyboard.println(api2Json.readKVP(piccChallenge, "USERNAME"));

    api2Json.storeKVP(piccChallenge, "PASSWORD", SECRET_PASSWORD);//only needs to be stored once.
    Keyboard.println(api2Json.readKVP(piccChallenge, "PASSWORD"));
